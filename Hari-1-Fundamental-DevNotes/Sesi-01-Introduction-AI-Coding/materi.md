...
### Pengenalan Studi Kasus: DevNotes

Mulai Sesi 2, seluruh latihan 3 hari akan dirangkai membangun satu aplikasi: **DevNotes** — catatan pembelajaran teknis untuk developer. Detail lengkap (FR, mockup, model data) ada di [`/project-brd.md`](../../project-brd.md).

Ringkas:

- **Hari 1** — peserta bangun web statis DevNotes (HTML/CSS/JS, data dummy + localStorage).
- **Hari 2** — migrasi ke Next.js + Supabase + Vercel, BE punya API & auth.
- **Hari 3** — FE Next.js konsumsi BE, deploy final, demo end-to-end.

### Diskusi Kelompok (35')

Pertanyaan untuk dipecah ke kelompok kecil (4–5 orang):

1. **Use case pribadi**: di pekerjaan harian Anda, fitur DevNotes mana yang akan Anda pakai sendiri? (mis. catatan postmortem incident, learning log).
2. **Adaptasi**: kalau DevNotes harus jadi internal tool di perusahaan Anda, fitur apa yang akan Anda tambah/buang dari MVP? Mengapa?
3. **Risiko AI**: aspek mana dari DevNotes yang **tidak boleh** sepenuhnya digenerate AI tanpa review ketat? (mis. RLS policy, sanitasi markdown).

Setiap kelompok presentasi 2 menit. Output diskusi akan dipakai sebagai bahan refleksi Sesi 4 (build feature).
